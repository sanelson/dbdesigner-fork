{
  ...
  Copyright (c) 2002-2008 by Vadim V.Lopushansky
...
type
  ...
  public
    constructor Create(AObjectNameTemplateInfo: PObjectNameTemplateInfo; const DbQuote: AnsiString);
    destructor Destroy; override;
    procedure DecodeObjectFullName(const ObjectFullName: AnsiString;
      var CatalogName, SchemaName, ObjectName: AnsiString);
    function EncodeObjectFullName(const CatalogName, SchemaName, ObjectName: AnsiString): AnsiString;
...
//==================================
  ...
  DbmsObjectNameTemplateInfo: array[TDbmsType] of PObjectNameTemplateInfo = (
    ...
    nil, //eDbmsTypeMsSqlServer2005Up
    nil, //eDbmsTypeIbmDb2
...
constructor TObjectNameParser.Create(
  ...
  const DbQuote: AnsiString
  );
var
  vRegexpQuote: AnsiString;
  i: integer;
const
  cRegExprSpesialSymbols = '[]\^.$*+?{},&()/|:=!';
begin
{$IFDEF _TRACE_CALLS_}LogEnterProc(ClassName + '.' + 'Create', 'DbQuote=<' + DbQuote + '>');
  try try{$ENDIF _TRACE_CALLS_}
      ...
      if // #0 identify template unsupported correction in runtime
        ...
        (DbQuote <> #0) and
        (fObjectNameTemplateInfo.QuoteTemplate <> DbQuote) then
      begin
  ...
  // Empty parts of templates
    // catalog:
      fEmptyCatalogTemplate := StringReplace(fObjectNameTemplateInfo.CatalogTemplate, #1, '', [rfIgnoreCase]);
    // schema:
      fEmptySchemaTemplate := StringReplace(fObjectNameTemplateInfo.SchemaTemplate, #2, '', [rfIgnoreCase]);
    // object:
      fEmptyObjectTemplate := StringReplace(fObjectNameTemplateInfo.ObjectTemplate, #3, '', [rfIgnoreCase]);
...
{$IFDEF _TRACE_CALLS_}
  ...
  finally LogExitProc(ClassName + '.' + 'Create. LasError=' + IntToStr(fRegExpr.LastError));
  end;
{$ENDIF _TRACE_CALLS_}
end;
        ...
        begin
          ...
          begin
            if (MathesIndexes^[i] > 0) and
              ...
              (fRegExpr.MatchPos[MathesIndexes^[i]] > 0) then
            begin
...
{$IFDEF _TRACE_CALLS_}
  ...
  finally LogExitProc(ClassName + '.' + 'DecodeObjectFullName', 'CatalogName, SchemaName, ObjectName=' + CatalogName +
...
function TObjectNameParser.EncodeObjectFullName(const CatalogName,
  SchemaName, ObjectName: AnsiString): AnsiString;
var
  S: AnsiString;
  i: integer;
begin
{$IFDEF _TRACE_CALLS_}LogEnterProc(ClassName + '.' + 'EncodeObjectFullName', 'CatalogName, SchemaName, ObjectName=' +
  ...
  // CATALOG:
      ...
      else if fObjectNameTemplateInfo.RequiredParts[idxCatalog] then
        S := fEmptyCatalogTemplate
      else
        S := '';
  ...
  // SCHEMA:
      ...
      else if fObjectNameTemplateInfo.RequiredParts[idxSchema] then
        S := fEmptySchemaTemplate
      else
        S := '';
  ...
  // OBJECT:
      ...
      else if fObjectNameTemplateInfo.RequiredParts[idxObject] then
        S := fEmptyObjectTemplate
      else
        S := '';
  ...
  // Prepare result - remove leading empty parts:
      for i := 0 to 2 do
      begin
        if pos(fEmptyCatalogTemplate, Result) = 1 then
          Result := Copy(Result,
            Length(fEmptyCatalogTemplate) + 1,
            Length(Result) - Length(fEmptyCatalogTemplate)
            );
        if pos(fEmptySchemaTemplate, Result) = 1 then
          Result := Copy(Result,
            Length(fEmptySchemaTemplate) + 1,
            Length(Result) - Length(fEmptySchemaTemplate)
            );
        if pos(fEmptyObjectTemplate, Result) = 1 then
          Result := Copy(Result,
            Length(fEmptyObjectTemplate) + 1,
            Length(Result) - Length(fEmptyObjectTemplate)
            );
      end;
{$IFDEF _TRACE_CALLS_}
    except on e: exception do
