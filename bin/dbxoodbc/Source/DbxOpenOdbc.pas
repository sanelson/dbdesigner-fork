...
const
  DbxOpenOdbcVersion = '3.100 (2008-02-11): Beta';
...
function getSQLDriverODBC(sVendorLib: PAnsiChar; sResourceFile: PAnsiChar; out Obj): SQLResult; stdcall;
// priority unicode odbc api
function getSQLDriverODBCAW(sVendorLib: PAnsiChar; sResourceFile: PAnsiChar; out Obj): SQLResult; stdcall;
...
type
  ...
  protected
    ...
    fDBXVersion: Integer; // for dbx3 and more
    fClientVersion: Integer; // Auto detection of the client supporting fldWIDESTRING, fldstWIDEMEMO
  ...
  protected
    ...
    procedure OdbcCheck(OdbcCode: SqlReturn; const OdbcFunctionName: AnsiString; DbxConStmt: PDbxConStmt); {$IFDEF _INLINE_} inline; {$ENDIF}
  ...
  TStmtCommandStatus = set of (scsStmtBinded, scsIsCursor);
  ...
  protected
    ...
    // DoPrepare, DoExecute, DoExecuteImmediate:
    //   bUseUnicodeOdbc == True only when SQL is PWideChar
...
begin
  ...
  begin
    ...
    begin
      ...
      StrCopy(pOut, PAnsiChar(IntToStr(iExp)));
      if iExp < 10 then
        Inc(pOut)
      else
        Inc(pOut, 2);
  ...
  case aSqlType of
    ...
    SQL_BIGINT:
      begin
        ...
        begin
          // Default code:
          {$IFDEF _DBX3_}
          if SqlConnectionOdbc.fOwnerDbxDriver.fDBXVersion >= 3 then
            DbxType := fldINT64
          else
          {$ENDIF}
            DbxType := fldINT32;
    ...
    begin
      ...
      begin
        ...
        else if (aSqlType = SQL_MSSQL_VARIANT) // sql_variant: SELECT value FROM "dbo"."sysproperties"
          and (SqlConnectionOdbc.fOdbcDriverType in [eOdbcDriverTypeMsSqlServer, eOdbcDriverTypeMsSqlServer2005Up])
        then
        begin
          DbxType := fldZSTRING;
        end;
...
function FormatParameter(Parameter: TOdbcBindParam): AnsiString;
var
  WS: WideString;
begin
...
begin
  ...
  case CheckCode of
    ...
    begin
      ...
      RetrieveOdbcErrorInfo(CheckCode, HandleType, Handle, DbxConStmt, Connection, Command, Cursor, 0, maxErrorCount);
      raise EDbxOdbcError.Create(fOdbcErrorLines.Text);
    end;
  end;
  {$IFDEF _TRACE_CALLS_}
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if bClearErrorCount = 0 then
          fOdbcErrorLines.Add(CheckCodeText)
        else
        if bClearErrorCount > 0 then
...
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSqlDriverOdbc.GetOption', ['eDOption =',cSQLDriverOption[xeDOption]]); {$ENDIF _TRACE_CALLS_}
    ...
    case xeDOption of
      ...
      xeDrvVersion:
        begin
          fClientVersion := 3;
          {$IFDEF _DBX3_}
          if fDBXVersion >= 3 then
          begin
            if PropValue <> nil then
              Length := min(System.Length(AnsiString(DBXDRIVERVERSION30)), MaxLength)
            else
              Length := 0;
            if Length > 0 then
              StrLCopy(PropValue, PAnsiChar(DBXDRIVERVERSION30), Length);
            //else
            //  Result := DBXERR_INVALIDPARAM;
          end
          else
...
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSqlDriverOdbc.getSQLConnection'); {$ENDIF _TRACE_CALLS_}
  try
...
var
  xeDOption: TXSQLDriverOption absolute eDOption;
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSqlDriverOdbc.SetOption', ['eDOption =',cSQLDriverOption[xeDOption]]); {$ENDIF _TRACE_CALLS_}
  try
    Result := DBXERR_NONE;
    case xeDOption of
      xeDrvBlobSize:
...
begin
  {$IFDEF _DBX3_}
  if fOwnerDbxDriver.fDBXVersion >= 3 then
    ...
    begin
      ...
      if OdbcRetcode <> OdbcApi.SQL_SUCCESS then
        fOwnerDbxDriver.OdbcCheck(OdbcRetcode, 'SQLAllocHandle(SQL_HANDLE_STMT)',
          SQL_HANDLE_STMT, fDbxConStmt.fHCon, @fDbxConStmt);
    ...
    begin
      ...
      if (OdbcRetcode <> OdbcApi.SQL_SUCCESS) and (not iDbxConStmt.fDeadConnection) then
        fOwnerDbxDriver.OdbcCheck(OdbcRetcode, 'SQLFreeHandle(SQL_HANDLE_STMT)',
          SQL_HANDLE_STMT, HStmt, @iDbxConStmt);
    end;
...
begin
  ...
  begin
    ...
    if (OdbcRetcode <> OdbcApi.SQL_SUCCESS) and (not fConnectionClosed) then
      OdbcCheck(OdbcRetcode, 'SQLFreeHandle(SQL_HANDLE_STMT)', nil);
    HStmt := SQL_NULL_HANDLE;
    dec(fSqlHStmtAllocated);
  end;
  {$IFDEF _TRACE_CALLS_}
...
//  const OdbcFunctionName: AnsiString; DbxConStmt: PDbxConStmt);
begin
  fOwnerDbxDriver.OdbcCheck(OdbcCode, OdbcFunctionName, SQL_HANDLE_DBC,
    GetCurrentConnectionHandle, DbxConStmt, Self);
end;
...
var
  ...
  i: Integer;
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSqlConnectionOdbc.beginTransaction', ['TranID =', GetTransactionDescStr(pTTransactionDesc(TranID))]); {$ENDIF _TRACE_CALLS_}
  {
...
var
  ...
  // ---
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSqlConnectionOdbc.commit', ['TranID =', TranID]); {$ENDIF _TRACE_CALLS_}
  with fOwnerDbxDriver.fOdbcApi do
  try
    if fStatementPerConnection = 0  then
    begin
...
var
  ...
  // ---
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSqlConnectionOdbc.rollback', ['TranID =', TranID]); {$ENDIF _TRACE_CALLS_}
  with fOwnerDbxDriver.fOdbcApi do
  try
    if fStatementPerConnection = 0  then
    begin
...
var
 ...
 {$ENDIF IFDEF _MIXED_FETCH_}
  ...
  // ---
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSqlConnectionOdbc.connect', ['ConnectString =', fConnConnectionString, 'ServerName=', ServerName, 'UserName =', UserName, 'Password =', Password]); {$ENDIF _TRACE_CALLS_}
  Result := DBXERR_NONE;
  if fConnected then
    exit;
...
{$ENDIF}
    ...
    *)
    ...
    //todo (need check): if (Self.fOdbcDriverType in [eOdbcDriverTypeMsSqlServer, eOdbcDriverTypeMsSqlServer2005Up]) then
    begin
      tmpS := fOdbcReturnedConnectString;
    ...
    {$ENDIF}
    {
    else if (fOwnerDbxDriver.fClientVersion >= 3) then
...
{$IFDEF _TRACE_CALLS_}
  ...
  for i := Low(TConnectionOption) to High(TConnectionOption) do
  begin
    LogInfoProc([IntToStr(27+Byte(i))+'-'+cConnectionOptionsNames[i]+' =',
...
begin
  ...
  try
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  if SetOption(
                      TSQLConnectionOption(xeConnDecimalSeparator), Integer(PAnsiChar(string(S[1])))
                    ) = DBXERR_NONE then
                  begin
    ...
    {$IFDEF _TRACE_CALLS_}
      ...
      finally LogExitProc('TSqlConnectionOdbc.RetrieveDbmsOptions'); end;
    {$ENDIF _TRACE_CALLS_}
  finally
    if iCursor <> nil then
    try
...
begin
  ...
  //
    ...
    (StrLComp(pBuffer, 'SQLSRV', 6) = 0)    // SQL Server Microsoft Corporation ODBC Driver
...
begin
  ...
  StrCopy(Error, PAnsiChar(fConnectionErrorLines.Text));
...
{$IFDEF _TRACE_CALLS_}
  ...
  begin
    case xeDOption of
      xeConnAutoCommit,
      xeConnBlockingMode:
        Result := BoolToStr(Boolean(PropValue^));
      xeConnBlobSize:
        Result := IntToStr(Integer(PropValue^));
      xeConnTxnIsoLevel:
      ...
      xeConnDecimalSeparator:
        ...
        Result := IntToStr(Longint(PropValue^));
    end; //of: case
  end;
{$ENDIF _TRACE_CALLS_}
  // ---
...
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSqlConnectionOdbc.getSQLCommand'); {$ENDIF _TRACE_CALLS_}
  try
...
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSqlConnectionOdbc.getSQLMetaData'); {$ENDIF _TRACE_CALLS_}
  try
...
var
  ...
  begin
    if (fOwnerDbxDriver.fDBXVersion < 3) and  (fOwnerDbxDriver.fClientVersion >= 3) then
      // Delphi 2006 put parameter as unicode
      ...
      xeConnTDSPacketSize:
        begin
          if lValue >= cNetwrkPacketSizeMin then
            SetConnectionOption(fConnectionOptions, nil, coNetwrkPacketSize, IntToStr(lValue), Self);
        end;
      xeConnClientHostName:
        begin
...
var
  OdbcRetcode: OdbcApi.SqlReturn;
begin
  {$IFDEF _TRACE_CALLS_} try try LogEnterProc('TSqlCommandOdbc.CloseStmt'); {$ENDIF _TRACE_CALLS_}
  fPreparedOnly := False;
  fExecutedOk := False;
  if (fHStmt <> SQL_NULL_HANDLE) then
  with fOwnerDbxDriver.fOdbcApi do
...
procedure TSqlCommandOdbc.OdbcCheck;//(OdbcCode: SqlReturn; const OdbcFunctionName: AnsiString);
begin
  fOwnerDbxDriver.OdbcCheck(OdbcCode, OdbcFunctionName, SQL_HANDLE_STMT, fHStmt,
    fDbxConStmtInfo.fDbxConStmt, fOwnerDbxConnection, Self);
end;
...
begin
  ...
  {$IFDEF _TRACE_CALLS_}
    ...
    finally LogExitProc('TSqlCommandOdbc.DoSQLMoreResults'); end;
  {$ENDIF _TRACE_CALLS_}
end;
...
begin
  ...
  try
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            {$IFDEF _DBX3_}
            if fOwnerDbxDriver.fDBXVersion >= 3 then
              Pointer(Cursor) := Create_ISQLCursor30(Self)
            else
            {$ENDIF}
    ...
    begin
      ...
      //  or (fOwnerDbxConnection.fOdbcDriverType = eOdbcDriverTypeMsSqlServer) )
      //then
      if (fStoredProc = 0) then
        fStoredProc := 2;
      {+.}
  ...
  {$IFDEF _TRACE_CALLS_}
    ...
    finally LogExitProc('TSqlCommandOdbc.DoExecute'); end;
  {$ENDIF _TRACE_CALLS_}
end;
...
var
  ...
  OdbcRowsAffected: SqlInteger;
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSqlCommandOdbc.DoExecuteImmediate', ['SQL =', SQL, 'UseUnicodeOdbc =', bUseUnicodeOdbc]); {$ENDIF _TRACE_CALLS_}
  Cursor := nil;
  fOdbcRowsAffected := 0;
  with fOwnerDbxDriver.fOdbcApi do
  try
  ...
  {$IFDEF _TRACE_CALLS_}
    ...
    finally LogExitProc('TSqlCommandOdbc.DoExecuteImmediate'); end;
  {$ENDIF _TRACE_CALLS_}
end;

function TSqlCommandOdbc.ExecuteImmediate;//(SQL: PAnsiChar;  var Cursor: ISQLCursor25): SQLResult;
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSqlCommandOdbc.ExecuteImmediate', ['SQL =', SQL]); {$ENDIF _TRACE_CALLS_}
...
begin
  ...
  StrCopy(Error, PAnsiChar(fOwnerDbxConnection.fConnectionErrorLines.Text));
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        IsBlank := 1;
        if not (fDbxType in [fldZSTRING, fldWIDESTRING]) then
          exit;
        vData := nil;
      end
      else
...
var
  OdbcRetcode: OdbcApi.SqlReturn;
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSqlCommandOdbc.DoPrepare', ['SQL =', SQL, 'ParamCount =', ParamCount,
     'UpdateParams =', UpdateParams, 'UseUnicodeOdbc =', bUseUnicodeOdbc, 'StoredProc =', fStoredProc]); {$ENDIF _TRACE_CALLS_}
  if SQL = nil then
  begin
    Result := DBXERR_INVALIDPARAM;
    exit;
  ...
  try
    ...
    begin
      ...
      fOwnerDbxConnection.fObjectNameParser.DecodeObjectFullName(
       SQL, fCatalogName, fSchemaName, fSql);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if OdbcRetcode <> OdbcApi.SQL_SUCCESS then
          begin
            fSqlPrepared := fSqlPreparedW;
            OdbcCheck(OdbcRetcode, 'SQLPrepareW');
          end;
        end
        else
        begin
  ...
  {$IFDEF _TRACE_CALLS_}
    ...
    finally LogExitProc('TSqlCommandOdbc.DoPreparee'); end;
  {$ENDIF _TRACE_CALLS_}
end;

function TSqlCommandOdbc.prepare;//(SQL: PAnsiChar; ParamCount: Word): SQLResult;
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSqlCommandOdbc.prepare', ['SQL =', SQL, 'ParamCount =', ParamCount, 'StoredProc =', fStoredProc]); {$ENDIF _TRACE_CALLS_}
  if SQL = nil then
  begin
    Result := DBXERR_INVALIDPARAM;
    exit;
...
var
  ...
  begin
    case xeSqlCommandOption of
      xeCommBlockRead:
        Result := BoolToStr(Boolean(ulValue));
      xeCommCursorName:
        Result := StrPas(PAnsiChar(ulValue));
      xeCommStoredProc:
        Result := BoolToStr(Boolean(ulValue));
      xeCommPackageName:
        Result := StrPas(PAnsiChar(ulValue));
      ...
      case uLogType of
          ...
          *)
          begin
            ...
            {begin:}
              ...
              begin // NOT NULL DATA
                if (fOwnerDbxConnection.fConnectionOptions[coEmptyStrParam] = osOn)
                  and ( eParamType in [paramIN, paramINOUT] )
                  and
                  (  // unicode check
                     ( bUnicodeString and (PWideChar(pBuffer)^ = cNullWideChar) )
                     or
                     // ansi char check
                     ( PAnsiChar(pBuffer)^ = cNullAnsiChar )
                  ) then
                begin
                  pBuffer := nil;
                  Length := 0;
            ...
            if {aOdbcBindParam.}fOdbcParamCbColDef > 255 then
            begin
        ...
        fldDATETIME:
          begin
            ...
            //todo (need check): if (fOwnerDbxConnection.fOdbcDriverType in [eOdbcDriverTypeMsSqlServer, eOdbcDriverTypeMsSqlServer2005Up]) then
            begin
...
//              aTimeStamp := TTimeStamp(pBuffer^);
        ...
        fldFMTBCD:
          begin
            ...
            //todo (need check): if (fOwnerDbxConnection.fOdbcDriverType in [eOdbcDriverTypeMsSqlServer, eOdbcDriverTypeMsSqlServer2005Up]) and
          ...
          begin
            ...
            case uSubType of
              ...
              fldstMEMO, fldstFMTMEMO, fldstHMEMO, fldstWIDEMEMO:
                begin
                  if uSubType <> fldstWIDEMEMO then
                  begin
                    fOdbcParamCType := SQL_C_CHAR;
                    fOdbcParamSqlType := SQL_LONGVARCHAR;
                  end
...
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSQLMetaDataOdbc.getColumns'); {$ENDIF _TRACE_CALLS_}
  {$IFDEF _DBX3_}
  if fOwnerDbxConnection.fOwnerDbxDriver.fDBXVersion >= 3 then
  ...
  try
    ...
    {$IFDEF _DBX3_}
    if fOwnerDbxConnection.fOwnerDbxDriver.fDBXVersion >= 3 then
      Pointer(Cursor) := Get_ISQLMetaData30(TSqlCursorMetaDataColumns3(aCursor))
    else
    {$ENDIF}
      Cursor := aCursor;
    Result := DBXERR_NONE;
...
begin
  ...
  StrCopy(Error, PAnsiChar(fMetaDataErrorLines.Text));
...
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSQLMetaDataOdbc.getIndices', ['TableName =', TableName, 'IndexType =', IndexType]); {$ENDIF _TRACE_CALLS_}
  {$IFDEF _DBX3_}
  if fOwnerDbxConnection.fOwnerDbxDriver.fDBXVersion >= 3 then
  ...
  try
    ...
    {$IFDEF _DBX3_}
    if fOwnerDbxConnection.fOwnerDbxDriver.fDBXVersion >= 3 then
      Pointer(Cursor) := Get_ISQLMetaData30(TSqlCursorMetaDataIndexes3(aCursor))
    else
    {$ENDIF}
      Cursor := aCursor;
    Result := DBXERR_NONE;
...
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSQLMetaDataOdbc.getProcedureParams', ['ProcName =', ProcName, 'ParamName =', ParamName]); {$ENDIF _TRACE_CALLS_}
  {$IFDEF _DBX3_}
  if fOwnerDbxConnection.fOwnerDbxDriver.fDBXVersion >= 3 then
  ...
  try
    ...
    {$IFDEF _DBX3_}
    if fOwnerDbxConnection.fOwnerDbxDriver.fDBXVersion >= 3 then
      Pointer(Cursor) := Get_ISQLMetaData30(TSqlCursorMetaDataProcedureParams3(aCursor))
    else
    {$ENDIF}
      Cursor := aCursor;
    Result := DBXERR_NONE;
...
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSQLMetaDataOdbc.getProcedures', ['ProcedureName =', ProcedureName, 'ProcType =', ProcType]); {$ENDIF _TRACE_CALLS_}
  {$IFDEF _DBX3_}
  if fOwnerDbxConnection.fOwnerDbxDriver.fDBXVersion >= 3 then
  ...
  try
    ...
    {$IFDEF _DBX3_}
    if fOwnerDbxConnection.fOwnerDbxDriver.fDBXVersion >= 3 then
      Pointer(Cursor) := Get_ISQLMetaData30(TSqlCursorMetaDataProcedures3(aCursor))
    else
    {$ENDIF}
      Cursor := aCursor;
    Result := DBXERR_NONE;
...
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSQLMetaDataOdbc.getTables', ['TableName =', TableName, 'TableType =', TableType]); {$ENDIF _TRACE_CALLS_}
  {$IFDEF _DBX3_}
  if fOwnerDbxConnection.fOwnerDbxDriver.fDBXVersion >= 3 then
  ...
  try
    ...
    {$IFDEF _DBX3_}
    if fOwnerDbxConnection.fOwnerDbxDriver.fDBXVersion >= 3 then
      Pointer(Cursor) := Get_ISQLMetaData30(TSqlCursorMetaDataTables3(aCursor))
    else
    {$ENDIF}
      Cursor := aCursor;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if (fOwnerDbxConnection.fOdbcDriverType in [eOdbcDriverTypeMsSqlServer, eOdbcDriverTypeMsSqlServer2005Up])
        ...
        case fSqlType of
          ...
          SQL_BIGINT:
            begin
              ...
              {$IFDEF _D9UP_}{$ENDREGION}{$ENDIF}
              {$IFDEF _DBX3_}
              if (fOwnerDbxDriver.fDBXVersion >= 3) and ((fOwnerDbxConnection.fConnectionOptions[coMapInt64ToBcd] = osOff)) then
              begin
                fDbxType := fldINT64;
                fOdbcHostVarType := SQL_C_SBIGINT;
                fOdbcHostVarSize := SizeOf(SqlBigInt);
...
{$IFDEF _D9UP_}{$ENDREGION}{$ENDIF}
          ...
          SQL_CHAR, SQL_VARCHAR, SQL_WCHAR, SQL_WVARCHAR, SQL_GUID:
            begin
              ...
              if bUnicodeString then
              begin
                if fOwnerDbxDriver.fClientVersion >= 3 then
                begin
...
//  bAllowFragmentation: Boolean = False; FirstChunkSize: Integer = 0);
var
  OdbcRetcode: OdbcApi.SqlReturn;
  aOdbcBindCol: TOdbcBindCol;
  ...
  begin
    ...
    begin
      ...
      {$IFDEF _TRACE_CALLS_}
      LogInfoProc('NextChunkSize = "' + IntToStr(BlobChunkSize) + '"');
      {$ENDIF _TRACE_CALLS_}
    end;
    //*)
  end;
...
begin
  ...
  begin
    ...
    {$IFDEF _TRACE_CALLS_}
    LogInfoProc('FirstChunkSize = "' + IntToStr(BlobChunkSize) + '"');
    {$ENDIF _TRACE_CALLS_}
    {/+2.01}
    PreviousBlobSize := 0;
...
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSqlCursorOdbc.getColumnNameLength', ['ColumnNumber =', ColumnNumber]); {$ENDIF _TRACE_CALLS_}
...
begin
  ...
  StrCopy(Error, PAnsiChar(fOwnerCommand.fOwnerDbxConnection.fConnectionErrorLines.Text));
...
begin
  ...
  begin
    ...
    bUnicodeString := (fDbxType = fldWIDESTRING) or ((fDbxSubType and fldstWIDEMEMO) <> 0) or (fDbxType = fldUNICODE);
...
var
  aOdbcBindCol: TOdbcBindCol;
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSqlCursorOdbc.isNullable', ['ColumnNumber =', ColumnNumber]); {$ENDIF _TRACE_CALLS_}
  aOdbcBindCol := TOdbcBindCol(fOdbcBindList[ColumnNumber - 1]);
...
//  aOdbcBindCol: TOdbcBindCol;
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSqlCursorOdbc.isReadOnly', ['ColumnNumber =', ColumnNumber]); {$ENDIF _TRACE_CALLS_}
  with fOwnerDbxDriver.fOdbcApi do
  try
...
var
  ...
  aOdbcBindCol: TOdbcBindCol;
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try LogEnterProc('TSqlCursorOdbc.isSearchable', ['ColumnNumber =', ColumnNumber]); {$ENDIF _TRACE_CALLS_}
  with fOwnerDbxDriver.fOdbcApi do
  try
    aOdbcBindCol := TOdbcBindCol(fOdbcBindList[ColumnNumber - 1]);
...
begin
  ...
  StrCopy(Error, PAnsiChar(fSqlCursorErrorMsg.Text));
...
//      v_CHAR_OCTET_LENGTH := 0;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if bOrdinalPositionEmulate then
              inc(OrdinalPosition);
            aMetaProcedureParam := TMetaProcedureParam.Create(PAnsiChar('@fix_' + IntToStr(OrdinalPosition)));
            fProcColumnList.Add(aMetaProcedureParam);
            aMetaProcedureParam.fMetaProcedure := aMetaProcedure;
            ...
            case aMetaProcedureParam.fDataType of
              ...
              fldTIMESTAMP:
                sTypeName := 'TIMESTAMP';
              fldBCD, fldFMTBCD:
                sTypeName := Format('DECIMAL(%d,%d)', [aMetaProcedureParam.fPrecision, aMetaProcedureParam.fScale]);
              fldBOOL:
                sTypeName := 'BYTE';
              fldBLOB:
  ...
  {$IFDEF _TRACE_CALLS_}
    ...
    finally LogExitProc('TSqlCursorMetaDataProcedureParams.FetchProcedureParams', 'ProcList.Count = ' + IntToStr(fProcList.Count)); end;
  {$ENDIF _TRACE_CALLS_}
end;
...
{$ENDIF _TRACE_CALLS_}
  RegisterDbXpressLibA := DoRegisterDbXpressLibA;
  RegisterDbXpressLibAW := DoRegisterDbXpressLibAW;
